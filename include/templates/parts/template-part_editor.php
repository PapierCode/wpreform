...
if ( isset( $settings_pc['blockeditor-disabled'] ) ) {
	...
	add_filter( 'post_gallery', 'pc_post_gallery_custom', 10, 3 );
		...
		function pc_post_gallery_custom( $output, $atts, $instance ) {
			...
			// html contruction
			$return = '<ul class="gallery reset-list">';
				...
				foreach ( $img_id_list as $img_id ) {
					...
					if ( !isset($large_datas) ) { $large_datas = wp_get_attachment_image_src($value,'full'); }
						...
						$return .= '<a class="gallery-link" href="'.$large_datas.'" data-gl-caption="'.$caption.'" data-gl-responsive="'.$medium_datas.'" title="Afficher l\'image">';
						$return .= '<img class="gallery-img" src="'.$thumbnail_datas[0].'" width="'.$thumbnail_datas[1].'" height="'.$thumbnail_datas[2].'" alt="'.$alt.'" loading="lazy"/>';
						$return .= '<span class="gallery-ico">'.pc_svg('zoom').'</span>';
						$return .= '</a>';
						$return .= '</li>';
